...
import { GlobalMacFilterRule, fetchGlobalMacFilterDefault, saveGlobalMacFilterDefault, fetchGlobalMacFilterRules, saveGlobalMacFilterRules, applyGlobalMacFilterSettings } from '../../utils/api';
import { GlobalMacFilterEditModal } from '../../components/GlobalMacFilterEditModal';
import { useAlert } from '../../utils/AlertContext';
import { SquareSwitch } from '../../components/UIComponents';

export const GlobalMacFilteringPage: React.FC = () => {
  const { showAlert } = useAlert();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [rules, setRules] = useState<GlobalMacFilterRule[]>([]);
    ...
    const load = async () => {
      try {
        const [defaultRes, rulesRes] = await Promise.all([
          fetchGlobalMacFilterDefault(),
          fetchGlobalMacFilterRules()
        ]);
        
        if (defaultRes && (defaultRes.success || defaultRes.cmd === 30) && defaultRes.datas && defaultRes.datas.length > 0) {
          setMode(defaultRes.datas[0].acceptAll ? 'blacklist' : 'whitelist');
        }
        
        if (rulesRes && (rulesRes.success || rulesRes.cmd === 23)) {
          setRules(rulesRes.datas || []);
        }
      } catch (e) {
        console.error("Failed to load MAC filter rules", e);
  ...
  const handleModalSave = (rule: GlobalMacFilterRule) => {
  ...
  const handleGlobalSave = async () => {
    ...
    try {
      ...
      const rulesDatas = rules.map((r, index) => ({
        ...r,
        enableLink: !isBlacklist // enableLink is false for blacklist, true for whitelist
      }));

      const [defaultRes, rulesRes] = await Promise.all([
        saveGlobalMacFilterDefault(defaultDatas),
        saveGlobalMacFilterRules(rulesDatas)
      ]);

      if ((defaultRes?.success || defaultRes?.cmd === 30) && (rulesRes?.success || rulesRes?.cmd === 23)) {
        await applyGlobalMacFilterSettings();
    ...
    <div className="w-full animate-fade-in py-6">
      ...
      <div className="border-t border-gray-200 mt-2">
        ...
        <div className="grid grid-cols-12 py-4 border-b border-gray-100">
          ...
          <div className="col-span-2 font-bold text-sm text-black">IPv4/IPv6</div>
          <div className="col-span-3 font-bold text-sm text-black">MAC Address</div>
          <div className="col-span-2 font-bold text-sm text-black">Remark</div>
        ...
        {rules.length > 0 ? rules.map((item, index) => (
          <div key={index} className="grid grid-cols-12 py-4 border-b border-gray-100 hover:bg-gray-50 transition-colors items-center">
            ...
            <div className="col-span-2 text-sm text-black">{index + 1}</div>
            <div className="col-span-2 text-sm text-black">{item.ippro === 'IPV4V6' ? 'IPv4/IPv6' : item.ippro}</div>
            <div className="col-span-3 text-sm text-black uppercase">{item.mac}</div>
            <div className="col-span-2 text-sm text-black truncate pr-2">{item.remark}</div>
