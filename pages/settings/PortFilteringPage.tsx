...
import { PortFilterRule, fetchPortFilterDefault, savePortFilterDefault, fetchPortFilterRules, savePortFilterRules, applyPortFilterSettings } from '../../utils/api';
import { PortFilterEditModal } from '../../components/PortFilterEditModal';
import { useAlert } from '../../utils/AlertContext';
import { SquareSwitch } from '../../components/UIComponents';

export const PortFilteringPage: React.FC = () => {
  const { showAlert } = useAlert();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [rules, setRules] = useState<PortFilterRule[]>([]);
    ...
    const load = async () => {
      try {
        const [defaultRes, rulesRes] = await Promise.all([
          fetchPortFilterDefault(),
          fetchPortFilterRules()
        ]);
        
        if (defaultRes && (defaultRes.success || defaultRes.cmd === 28) && defaultRes.datas && defaultRes.datas.length > 0) {
          setMode(defaultRes.datas[0].acceptAll ? 'blacklist' : 'whitelist');
        }
        
        if (rulesRes && (rulesRes.success || rulesRes.cmd === 21)) {
          setRules(rulesRes.datas || []);
        }
      } catch (e) {
        console.error("Failed to load Port filter rules", e);
  ...
  const handleModalSave = (rule: PortFilterRule) => {
  ...
  const handleGlobalSave = async () => {
    ...
    try {
      ...
      const [defaultRes, rulesRes] = await Promise.all([
        savePortFilterDefault(defaultDatas),
        savePortFilterRules(rulesDatas)
      ]);

      if ((defaultRes?.success || defaultRes?.cmd === 28) && (rulesRes?.success || rulesRes?.cmd === 21)) {
        await applyPortFilterSettings();
    ...
    <div className="w-full animate-fade-in py-6">
      ...
      <div className="border-t border-gray-200 mt-2">
        ...
        <div className="grid grid-cols-12 py-4 border-b border-gray-100">
          ...
          <div className="col-span-2 font-bold text-sm text-black">Protocol</div>
          <div className="col-span-2 font-bold text-sm text-black">Port</div>
        ...
        {rules.length > 0 ? rules.map((item, index) => (
          <div key={index} className="grid grid-cols-12 py-4 border-b border-gray-100 hover:bg-gray-50 transition-colors items-center">
            ...
            <div className="col-span-2 text-sm text-black">{item.protocol}</div>
