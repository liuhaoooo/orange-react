...
export const DdosProtectionPage: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetchDdosSettings();
        if (res && (res.success || res.cmd === 275)) {
          setEnabled(res.ddosSwich === '1');
        }
      } catch (e) {
        console.error("Failed to load DDoS settings", e);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await saveDdosSettings(enabled ? '1' : '0');
      if (res && (res.success || res.cmd === 275)) {
