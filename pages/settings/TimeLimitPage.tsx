...
import { TimeLimitRule, fetchTimeLimitRules, saveTimeLimitRules, applyTimeLimitSettings, fetchParentalModeSettings } from '../../utils/api';
import { TimeLimitEditModal } from '../../components/TimeLimitEditModal';
...
export const TimeLimitPage: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (rulesRes && (rulesRes.success || rulesRes.cmd === 385)) {
          setRules(rulesRes.datas || []);
        }
      } catch (e) {
        console.error("Failed to load Time Limit rules", e);
  ...
  const handleModalSave = (newRules: TimeLimitRule[]) => {
  ...
  const handleGlobalSave = async () => {
    setSaving(true);
    try {
      const rulesRes = await saveTimeLimitRules(rules);

      if (rulesRes?.success || rulesRes?.cmd === 385) {
        await applyTimeLimitSettings();
  ...
  const formatDays = (daysString: string) => {
    ...
    return sortedDays.map(d => DAYS_MAP[d]).filter(Boolean).join(',');
  };
    ...
    return (
      <div className="w-full py-16 flex flex-col items-center justify-center animate-fade-in">
        ...
        <p className="text-gray-500 text-sm text-center max-w-md">
          Please enable Parental Mode first to configure Time Limit rules.
        </p>
      </div>
    );
  }
    ...
    <div className="w-full animate-fade-in py-6">
      <div className="border-t border-gray-200 mt-2">
        ...
        <div className="grid grid-cols-12 py-4 border-b border-gray-100">
          ...
          <div className="col-span-2 font-bold text-sm text-black">Limit Time</div>
          <div className="col-span-3 font-bold text-sm text-black">Limit Date</div>
        ...
        {rules.length > 0 ? rules.map((item, index) => (
          <div key={index} className="grid grid-cols-12 py-4 border-b border-gray-100 hover:bg-gray-50 transition-colors items-center">
            ...
            <div className="col-span-3 text-sm text-black truncate pr-2">{item.mac}</div>
            <div className="col-span-2 text-sm text-black">{item.startTime}-{item.endTime}</div>
            <div className="col-span-3 text-sm text-black truncate pr-2">{formatDays(item.scheduleDays)}</div>
